...
/** Preserve user input character-for-character for destructive visible-label checks. */
...
const ADMIN_TAP_PAGE_SIZE = 25;
const ADMIN_TAP_PAGE_MAX = 10_000;
...
function normalizePage(value: unknown): number {
  ...
  return Math.min(ADMIN_TAP_PAGE_MAX, parsed);
}

/** Normalize the bounded Tap list query before it reaches SQL. */
export function validateAdminTapPageQuery(input: unknown = {}): {
  readonly q: string;
  ...
  const rawState = typeof object.state === "string" ? object.state.trim().toLowerCase() : "all";
  const state = rawState as AdminTapPageState;
  if (!["all", "assigned", "unassigned", "disabled", "retired"].includes(state)) {
    throw validationError("state", "must be all, assigned, unassigned, disabled, or retired");
  }
  return { q, state, page: normalizePage(object.page) };
}

export function adminTapPageSize(): number {
  return ADMIN_TAP_PAGE_SIZE;
}

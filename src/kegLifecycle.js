...
export function activeLifecycle(db, tapId) {
  return db.prepare(`SELECT lifecycle_id, tap_id, batch_id, assignment_kind, started_at FROM keg_lifecycles
    WHERE tap_id = ? AND closed_at IS NULL`).get(tapId) || null;
}
...
export function assignKegLifecycle(db, {
  tapId, batchId = null, assignmentKind = batchId?.startsWith('custom:') ? 'custom' : 'brewfather',
  startedAt, closeReason = 'reassigned', updateTap
}) {
  const run = db.transaction(() => {
    const current = activeLifecycle(db, tapId);
    if (current) db.prepare('UPDATE keg_lifecycles SET closed_at = ?, close_reason = ? WHERE lifecycle_id = ?')
      .run(nowIso(startedAt), closeReason, current.lifecycle_id);
    if (updateTap) updateTap();
    const result = db.prepare(`INSERT INTO keg_lifecycles
      (tap_id, batch_id, assignment_kind, started_at) VALUES (?, ?, ?, ?)`)
      .run(tapId, batchId, assignmentKind, nowIso(startedAt));
    return db.prepare(`SELECT lifecycle_id, tap_id, batch_id, assignment_kind, started_at
      FROM keg_lifecycles WHERE lifecycle_id = ?`).get(result.lastInsertRowid);
  });
  return run();
}
...
export function closeKegLifecycle(db, { tapId, closedAt, closeReason = 'ended', updateTap }) {
  return db.transaction(() => {
    const current = activeLifecycle(db, tapId);
    if (current) db.prepare('UPDATE keg_lifecycles SET closed_at = ?, close_reason = ? WHERE lifecycle_id = ?')
      .run(nowIso(closedAt), closeReason, current.lifecycle_id);
    if (updateTap) updateTap();
    return current;
  })();
}
...
export function recordPour(db, { tapId, lifecycleId = null, volumePouredOz, timestamp }) {
  return db.transaction(() => {
    ...
    let lifecycle = null;
    if (lifecycleId !== null) {
      lifecycle = db.prepare(`SELECT lifecycle_id, batch_id FROM keg_lifecycles
        WHERE lifecycle_id = ? AND tap_id = ?`).get(lifecycleId, tapId);
      if (!lifecycle) throw new Error('Captured keg lifecycle does not belong to this tap');
    }
    return db.prepare(`INSERT INTO pour_logs

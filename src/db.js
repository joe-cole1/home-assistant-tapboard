...
const migrationNeedsApproval = appSchemaExists
  && preMigrationVersion < SCHEMA_VERSION
  && process.env.TAPBOARD_REQUIRE_MIGRATION_APPROVAL === 'true';
if (migrationNeedsApproval) {
  ...
  const countsMatch = Object.entries(current.counts).every(([table, count]) => approval.counts?.[table] === count)
    && Object.keys(approval.counts || {}).length === Object.keys(current.counts).length;
  if (current.integrity !== 'ok' || current.foreignKeyViolations !== 0
      || preOpenHasSQLiteSidecar
      || approval.databaseSha256 !== preOpenDatabaseSha256
      || approval.userVersion !== current.userVersion || !countsMatch) {
    db.close();
...
if (migrationNeedsApproval) {
  try {
    ...
    console.warn(`[Database migration] Schema migration succeeded, but the approval marker could not be archived: ${error.message}`);
  }
}
...
if (!existingSettings) {
  db.prepare(`INSERT INTO settings
    ...
    VALUES (1, 'modern_dark', 'oz', 'Hazardous Brews', 'Outfit', 'Inter', 1, ?, 0)`).run(unusablePinHash);
} else {
...
if (initialPin !== undefined && initialPin !== '') {
  if (!/^\d{4}$/.test(initialPin) || initialPin === '0000') {
    ...
    db.transaction(() => {
      db.prepare('UPDATE settings SET admin_pin_hash = ?, admin_pin_initialized = 1 WHERE id = 1').run(bcrypt.hashSync(initialPin, 10));

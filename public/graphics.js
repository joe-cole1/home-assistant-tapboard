...
export function renderTapGraphic(
  ...
  switch (style) {
    case 'nonic_pint':
      return renderServingGlass('nonic_pint', pctToBounds(fillPct, 45, 225), beerColor, isPouring, id);
    case 'shaker_pint':
      return renderServingGlass('shaker_pint', pctToBounds(fillPct, 45, 225), beerColor, isPouring, id);
    case 'pilsner_flute':
      return renderServingGlass('pilsner_flute', pctToBounds(fillPct, 30, 220), beerColor, isPouring, id);
    case 'stange':
      return renderServingGlass('stange', pctToBounds(fillPct, 40, 220), beerColor, isPouring, id);
    case 'goblet':
      return renderServingGlass('goblet', pctToBounds(fillPct, 45, 170), beerColor, isPouring, id);
    case 'teku':
      return renderServingGlass('teku', pctToBounds(fillPct, 45, 170), beerColor, isPouring, id);
    case 'thistle':
      return renderServingGlass('thistle', pctToBounds(fillPct, 45, 170), beerColor, isPouring, id);
    case 'ipa_glass':
      return renderServingGlass('ipa_glass', pctToBounds(fillPct, 35, 220), beerColor, isPouring, id);
    case 'tasting_glass':
      return renderServingGlass('tasting_glass', pctToBounds(fillPct, 55, 165), beerColor, isPouring, id);
    case 'stemmed_lager':
      return renderServingGlass('stemmed_lager', pctToBounds(fillPct, 40, 175), beerColor, isPouring, id);
    case 'wheat_glass':
...
function pctToBounds(pct, topY, bottomY) {
  return { liquidY: bottomY - (pct / 100) * (bottomY - topY), topY, bottomY };
}
...
function renderServingGlass(style, bounds, color, isPouring, id) {
  const shape = SERVING_GLASS_SHAPES[style];
  const isWater = color === 'WATER';
  const { liquidY, topY, bottomY } = bounds;
  const foamColor = color === '#080100' || color === '#130100' || color === '#200100' ? '#F5EBE6' : '#FFFDF5';
  const fill = isWater ? 'rgba(224, 247, 250, 0.25)' : color;

  return `
    <svg viewBox="0 40 160 185" preserveAspectRatio="xMidYMin meet" data-bottom-y="${bottomY}" data-top-rim-y="${topY}" class="tap-graphic-svg ${isPouring ? 'is-pouring' : ''}" role="img" aria-label="Beer serving glass" xmlns="http://www.w3.org/2000/svg">
      ...
      <g clip-path="url(#${style}Clip_${id})">
        ...
        ${!isWater ? `<g class="beer-cloud-foam" data-base-y="${liquidY}"><rect x="25" y="${liquidY - 7}" width="110" height="14" fill="${foamColor}" opacity="0.9" /><circle cx="62" cy="${liquidY - 5}" r="10" fill="${foamColor}" /><circle cx="82" cy="${liquidY - 7}" r="12" fill="${foamColor}" /><circle cx="102" cy="${liquidY - 5}" r="10" fill="${foamColor}" /></g>` : ''}
      </g>
      <path d="M 58 ${topY + 8} L 62 ${bottomY - 10}" stroke="#FFFFFF" stroke-width="2.5" opacity="0.35" />
    </svg>
  `;
}
...
function renderCornyKeg(pct, color, isPouring, id) {
  ...
  return `
    <svg viewBox="0 28 160 200" preserveAspectRatio="xMidYMin meet" data-bottom-y="220" data-top-rim-y="65" class="tap-graphic-svg ${isPouring ? 'is-pouring' : ''}" role="img" aria-label="Beer serving vessel" xmlns="http://www.w3.org/2000/svg">
      <defs>
...
function renderPintGlass(pct, color, isPouring, id) {
  ...
  return `
    <svg viewBox="0 40 160 190" preserveAspectRatio="xMidYMin meet" data-bottom-y="225" data-top-rim-y="45" class="tap-graphic-svg ${isPouring ? 'is-pouring' : ''}" role="img" aria-label="Beer serving vessel" xmlns="http://www.w3.org/2000/svg">
      <defs>
...
function renderWheatGlass(pct, color, isPouring, id) {
  ...
  return `
    <svg viewBox="0 25 160 200" preserveAspectRatio="xMidYMin meet" data-bottom-y="220" data-top-rim-y="30" class="tap-graphic-svg ${isPouring ? 'is-pouring' : ''}" role="img" aria-label="Beer serving vessel" xmlns="http://www.w3.org/2000/svg">
      <defs>
        ${
          !isWater
            ? `
...
function renderTulipGlass(pct, color, isPouring, id) {
  ...
  return `
    <svg viewBox="0 35 160 195" preserveAspectRatio="xMidYMin meet" data-bottom-y="170" data-top-rim-y="40" class="tap-graphic-svg ${isPouring ? 'is-pouring' : ''}" role="img" aria-label="Beer serving vessel" xmlns="http://www.w3.org/2000/svg">
      <defs>
...
function renderMug(pct, color, isPouring, id) {
  ...
  return `
    <svg viewBox="0 45 160 180" preserveAspectRatio="xMidYMin meet" data-bottom-y="215" data-top-rim-y="55" class="tap-graphic-svg ${isPouring ? 'is-pouring' : ''}" role="img" aria-label="Beer serving vessel" xmlns="http://www.w3.org/2000/svg">
      <defs>
...
function renderStoutGlass(pct, color, isPouring, id) {
  ...
  return `
    <svg viewBox="0 40 160 185" preserveAspectRatio="xMidYMin meet" data-bottom-y="220" data-top-rim-y="45" class="tap-graphic-svg ${isPouring ? 'is-pouring' : ''}" role="img" aria-label="Beer serving vessel" xmlns="http://www.w3.org/2000/svg">
      <defs>
...
function renderSnifter(pct, color, isPouring, id) {
  ...
  return `
    <svg viewBox="0 45 160 180" preserveAspectRatio="xMidYMin meet" data-bottom-y="175" data-top-rim-y="55" class="tap-graphic-svg ${isPouring ? 'is-pouring' : ''}" role="img" aria-label="Beer serving vessel" xmlns="http://www.w3.org/2000/svg">
      <defs>

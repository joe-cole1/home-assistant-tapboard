...
4. Verify `integrity_check`, `foreign_key_check`, schema version 3 and migration ledger, durable counts, lifecycle relationships, On Deck/custom-beverage records, administrator initialization, digest-only sessions, HA hydration where appropriate, and HTTP 200 from `/healthz`.

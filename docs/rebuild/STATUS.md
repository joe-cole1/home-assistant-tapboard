...
- Architecture: **FROZEN**
- Current phase: **Issue #68 Physical Kegs implemented; custom and Brewfather-linked beverages (#69) is next**
- Current branch: `main`
...
The list preserves the frozen implementation sequence with #85's local development surface between #67 and #68. Issues #67 and #85 are merged on `main`; issue #68 (Physical Kegs) is implemented, and issue #69 (Beverages) is next.
...
- one controlled `better-sqlite3` connection, foreign keys, transactional versioned migrations, exact version-3 schema validation, and resource closure;
- shared typed errors, centralized HTTP error mapping, explicit validation, and structured redacting logging;
- Foundation-, #67-, and #68-aware architecture guardrails and negative fixtures;
...
Schema version 3 contains typed #67 primitives, #68 Physical Keg tables (`kegs`, `keg_tare_history`, `keg_maintenance_records`), and no v1 data adoption. The PIN remains exactly four ASCII digits with limited stolen-verifier offline resistance; integration encryption relies on independent external `TAPBOARD_SECRET_KEY` material. Activity never recursively enters the outbox, and `not_queued_capacity` is a fixed-slot bounded degradation state rather than a storage-error classification. Domain beverages, fills, taps, provider adapters, delivery workers, telemetry, UI, and production deployment remain deferred.

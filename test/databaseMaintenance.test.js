...
function makeDatabase(directory) {
  ...
  database.prepare('INSERT INTO admin_sessions (token, expires_at) VALUES (?, ?)').run(`sha256:${'a'.repeat(64)}`, '2099-01-01T00:00:00.000Z');
...
test('online backup and disposable restore preserve schema, counts, integrity, and relationships', async () => {
  ...
  source.database.prepare('INSERT INTO pour_logs (tap_id, lifecycle_id, volume_poured_oz, timestamp, timestamp_epoch) VALUES (1, 10, 12, ?, ?)')
    .run('2026-08-01 12:00:00', 1_754_048_000);
  source.database.close();

  const backup = await createOnlineBackup({ sourceFile: source.file, backupDirectory: path.join(root, 'backups'), now: new Date('2026-08-01T16:00:00.000Z') });
  ...
  assert.deepEqual(verifyDatabaseFile(restored.file, { expectedVersion: 2, expectedCounts: backup.summary.counts }), restored.summary);
});
...
test('two-calendar-year pruning is backup-gated and preserves recent pours', async () => {
  ...
  const source = makeDatabase(path.join(root, 'source'));
  const insert = source.database.prepare('INSERT INTO pour_logs (tap_id, lifecycle_id, volume_poured_oz, timestamp, timestamp_epoch) VALUES (1, 10, 8, ?, ?)');
  ...
  const result = await backupThenPrunePourHistory({ sourceFile: source.file, backupDirectory: path.join(root, 'backups'), now });

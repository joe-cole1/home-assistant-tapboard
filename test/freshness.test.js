...
test('New badge is opt-in and limited to the first seven days on tap', () => {
  assert.equal(shouldShowNewBadge({ badge_fresh: 1, on_tap_at: new Date(now - NEW_BADGE_WINDOW_MS + 1).toISOString() }, now), true);
  assert.equal(shouldShowNewBadge({ badge_fresh: 1, on_tap_at: new Date(now - NEW_BADGE_WINDOW_MS).toISOString() }, now), false);

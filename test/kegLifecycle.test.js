...
import { activeLifecycle, assignKegLifecycle, captureActiveLifecycle, closeKegLifecycle, recordPour } from '../src/kegLifecycle.js';
...
test('assignment, reassignment, and close preserve immutable lifecycle history', () => {
  ...
  try {
    ...
    recordPour(db, { tapId: 1, lifecycleId: first.lifecycle_id, volumePouredOz: 10, timestamp: '2026-08-01T01:00:00.000Z' });
    const second = assignKegLifecycle(db, { tapId: 1, batchId: 'same-batch', startedAt: '2026-08-02T00:00:00.000Z' });
    recordPour(db, { tapId: 1, lifecycleId: second.lifecycle_id, volumePouredOz: 5, timestamp: '2026-08-02T01:00:00.000Z' });
    assert.notEqual(first.lifecycle_id, second.lifecycle_id);
    assert.equal(db.prepare('SELECT closed_at IS NOT NULL AS closed FROM keg_lifecycles WHERE lifecycle_id = ?').get(first.lifecycle_id).closed, 1);
    assert.deepEqual(db.prepare('SELECT lifecycle_id, volume_poured_oz FROM pour_logs ORDER BY id').all(), [
      { lifecycle_id: first.lifecycle_id, volume_poured_oz: 10 }, { lifecycle_id: second.lifecycle_id, volume_poured_oz: 5 }
    ]);
...
test('a pour completing after reassignment remains attributed to its start-time lifecycle', () => {
  ...
  try {
    ...
    recordPour(db, { tapId: 1, lifecycleId: captured.lifecycle_id, volumePouredOz: 6, timestamp: '2026-08-01T00:00:10.000Z' });
    assert.notEqual(first.lifecycle_id, second.lifecycle_id);
...
test('a pour without an assignment is durable and explicitly unscoped', () => {
  ...
  try {
    ...
    assert.deepEqual(db.prepare('SELECT volume_poured_oz, lifecycle_id, timestamp_epoch FROM pour_logs').get(),
      { volume_poured_oz: 4, lifecycle_id: null, timestamp_epoch: 1785542400 });
  } finally { db.close(); }
});
...
test('forecast includes only the open lifecycle, even for successive same-batch kegs', () => {
  ...
  try {
    ...
    recordPour(db, { tapId: 1, lifecycleId: activeLifecycle(db, 1).lifecycle_id, volumePouredOz: 120, timestamp: '2026-07-30T00:00:00.000Z' });
    assignKegLifecycle(db, { tapId: 1, batchId: 'same', startedAt: '2026-08-01T00:00:00.000Z' });
    recordPour(db, { tapId: 1, lifecycleId: activeLifecycle(db, 1).lifecycle_id, volumePouredOz: 12, timestamp: '2026-08-01T12:00:00.000Z' });
    assert.deepEqual(calculateKegKickForecast({ db, tapId: 1, currentOz: 120, nowMs: Date.parse('2026-08-02T00:00:00.000Z') }),
      { avgDailyOz: 12, estimatedDaysRemaining: 10, hasUsageData: true });
    assert.match(db.prepare(`EXPLAIN QUERY PLAN SELECT SUM(volume_poured_oz) FROM pour_logs
      WHERE lifecycle_id = ? AND timestamp_epoch >= ?`).all(activeLifecycle(db, 1).lifecycle_id, 0)
      .map(row => row.detail).join(' '), /pour_logs_lifecycle_epoch/);
  } finally { db.close(); }
});

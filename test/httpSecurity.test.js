...
test('JSON parser enforces media type, exact byte ceiling, malformed JSON, and required bodies', async () => {
  ...
  assert.equal(
    (await readJsonBody(request(exact, { 'content-type': 'application/json; charset = UTF-8' }))).value.length > 0,
    true
  );
  await assert.rejects(
    readJsonBody(request(jsonOfSize(MAX_JSON_BYTES + 1), { 'content-type': 'application/json' })),
    (error) => error.status === 413
  );
  await assert.rejects(
    readJsonBody(request('{}', { 'content-type': 'application/problem+json' })),
  ...
  const aborted = new Readable({
    read() {
      this.destroy(new Error('connection aborted'));
    }
  });
...
test('bodyless routes accept no body or an empty JSON object only', async () => {
  ...
  assert.deepEqual(
    await readEmptyJsonBody(request('{}', { 'content-type': 'application/json', 'content-length': '2' })),
    {}
  );
  await assert.rejects(
    readEmptyJsonBody(request('{"x":1}', { 'content-type': 'application/json', 'content-length': '7' })),
    (error) => error.status === 400
  );
  await assert.rejects(
    readEmptyJsonBody(request('{}', { 'content-type': 'text/plain', 'content-length': '2' })),
    (error) => error.status === 415
  );
});
...
test('origin policy is exact for direct and configured reverse-proxy origins', () => {
  ...
  assert.doesNotThrow(() =>
    enforceOrigin({ headers: { host: 'internal:3000', origin: 'https://tap.example' } }, 'https://tap.example')
  );
  for (const origin of ['null', 'http://evil.example', 'https://tap.example/', 'not-an-origin']) {
    assert.throws(
      () => enforceOrigin({ headers: { host: 'tap.local:3005', origin } }),
...
test('static resolution rejects encoded traversal, malformed encodings, NUL, backslashes, and prefix collisions', () => {
  ...
  assert.throws(
    () => resolvePublicPath(root, '/..%2farchitecture.md'),
    (error) => error.status === 403
  );
  assert.throws(
    () => resolvePublicPath(root, '/%E0%A4%A'),
    (error) => error.status === 400
  );
  assert.throws(
    () => resolvePublicPath(root, '/bad%00name'),
    (error) => error.status === 400
  );
  assert.throws(
    () => resolvePublicPath(root, '/..%5csecret'),
    (error) => error.status === 400
  );

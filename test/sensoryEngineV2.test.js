...
import { buildSensoryProfileV1 } from '../src/sensoryEngine.js';
import { buildSensoryModelV2, buildSensoryProfileV2, SENSORY_V2_RULES_VERSION } from '../src/sensoryEngineV2.js';
...
test('v2 uses canonical measured fields and continuous amount-aware formulas', () => {
  const model = buildSensoryModelV2({
    detail: {
      ...measurements({
        measured_ibu: 50,
        target_ibu: 10,
...
test('v2 precedence is manual then newest valid per-axis tasting then recipe then composable style', () => {
  const profile = buildSensoryProfileV2({
  ...
  assert.equal(profile.rules_version, SENSORY_V2_RULES_VERSION);
...
test('v2 distinguishes equal FG values using apparent attenuation', () => {
  const session = buildSensoryModelV2({ detail: measurements({ measured_og: 1.04, measured_fg: 1.014 }) });
  const barleywine = buildSensoryModelV2({ detail: measurements({ measured_og: 1.1, measured_fg: 1.014 }) });
...
test('v2 uses grist composition for roast and caps the SRM-only fallback', () => {
  ...
  const fallback = buildSensoryModelV2({ summary: { estimated_srm: 80 } });
...
test('v2 tartness prefers measured pH and otherwise accepts only recognized souring evidence', () => {
  const measured = buildSensoryModelV2({ detail: measurements({ measured_ph: 3.3 }) });
  const culture = buildSensoryModelV2({
    detail: { recipe: { ingredients: { yeasts: [{ name: 'Lallemand Philly Sour' }] } } }
  });
  const brett = buildSensoryModelV2({
    detail: { recipe: { ingredients: { yeasts: [{ name: 'Brettanomyces Bruxellensis' }] } } }
  });
  const explicit = buildSensoryModelV2({ detail: { recipe: { souring: 'Kettle sour' } } });
...
test('v2-only cached souring evidence cannot change the active v1 profile', () => {
  const detail = {
    batch: { sensory_v2_souring: 'Kettle sour' },
    recipe: { sensory_v2_souring: true }
  };
  assert.equal(buildSensoryProfileV1({ detail }).axes.tartness.value, null);
  assert.equal(buildSensoryProfileV2({ detail }).axes.tartness.value, 4);
});

test('v2 body combines gravity and adjunct percentage with diminishing returns', () => {
  const plain = buildSensoryModelV2({
    detail: {
      ...measurements({ measured_og: 1.06, measured_fg: 1.016 }),
      recipe: { ingredients: { fermentables: [{ name: 'Pale Malt', percentage: 100 }] } }
    }
  });
  const oats = buildSensoryModelV2({
    detail: {
      ...measurements({ measured_og: 1.06, measured_fg: 1.016 }),
      recipe: {
        ingredients: {
  ...
  const attenuationOnly = buildSensoryModelV2({
...
test('v2 style matching composes compound families and separates Tripel', () => {
  const sourIpa = buildSensoryProfileV2({ summary: { style: 'Sour IPA' } });
  const tripel = buildSensoryProfileV2({ summary: { style: 'Belgian Tripel' } });
  ...
  const sparseStout = buildSensoryProfileV2({ summary: { style: 'Imperial Stout', estimated_srm: 20 } });
...
test('v2 model excludes human layers and null-safe profiles invent nothing', () => {
  ...
  const empty = buildSensoryProfileV2({ summary: null, detail: null, override: null });
